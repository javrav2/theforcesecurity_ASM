...
- **scan_js_secrets**: Detect exposed API keys, passwords, tokens, and credentials in JavaScript files. Two modes: (1) domains — runs full pipeline: httpx liveness → katana JS discovery → regex + AI analysis; (2) urls — directly scans provided JS file URLs. Uses SecretFinder-style regex patterns (30+ patterns for AWS, Google, Stripe, Slack, JWT, etc.) plus optional AI deep analysis. Args: **domains** (comma-separated domains for full pipeline) OR **urls** (comma-separated JS file URLs for direct scan), use_ai (bool, default true), regex_only (bool, default false), max_js_urls (int, default 50). Example: scan_js_secrets(domains="example.com,sub.example.com"), scan_js_secrets(urls="https://example.com/static/app.js,https://example.com/bundle.js", regex_only=true).
"""
...
TOOL_PHASE_MAP = {
    ...
    "scan_js_secrets": ["informational", "exploitation", "post_exploitation"],
